      ...
      <section className="px-6 py-12 max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((cat, index) => (
            <div
              key={index}
              className={`relative group h-64 overflow-hidden rounded-sm cursor-pointer transition-all duration-300 ${cat.bgColor}`}
            >
              {/* Text Overlay */}
              <div className={`absolute top-6 left-6 z-10 ${cat.textColor}`}>
                <h2 className="text-2xl font-black leading-none uppercase tracking-tighter">
                  {cat.title}
                </h2>
                <p className="text-[10px] font-bold tracking-[0.2em] opacity-70 mt-1">
                  {cat.subtitle}
                </p>
              </div>

              {/* Image Styling - Positioned to the right/bottom like the reference */}
              <div className="absolute right-0 bottom-0 w-3/4 h-full transform translate-x-4 translate-y-4 group-hover:translate-x-2 group-hover:translate-y-2 transition-transform duration-500 ease-out">
                <img
                  src={cat.img}
                  alt={cat.title}
