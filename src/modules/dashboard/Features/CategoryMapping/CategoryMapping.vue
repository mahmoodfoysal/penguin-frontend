...
<template>
  ...
  <template v-for="(item, index) in categoryList" :key="index">
    <!-- Parent Category Row -->
    <tr>
      <td :rowspan="item.sub_categories.reduce((sum, sub) => sum + (sub.sub_sub_categories?.length || 1), 0)">{{ index + 1 }}</td>
      <td class="text-center" :rowspan="item.sub_categories.reduce((sum, sub) => sum + (sub.sub_sub_categories?.length || 1), 0)">{{ item.par_cat_id }}</td>
      <td :rowspan="item.sub_categories.reduce((sum, sub) => sum + (sub.sub_sub_categories?.length || 1), 0)">{{ item.par_cat_name }}</td>
    ...
    <template v-for="(subItem, subIndex) in item.sub_categories" :key="`sub-${subIndex}`">
      ...
      <template v-for="(subSubItem, subSubIndex) in subItem.sub_sub_categories?.slice(1) || []" :key="`subSub-${subIndex}-${subSubIndex}`">
        <tr>
